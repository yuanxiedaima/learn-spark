...
if __name__ == '__main__':
    ...
    #1加载user.txt，形成RDD1
    rdd1=sc.textFile('file:///export/pyworkspace/pyspark_dev/综合案例/data/user.txt')
    user_rdd=rdd1\
        .map(lambda x:x.split(','))\
        .map(lambda x:(int(x[0]),x[1],int(x[2]),x[3],x[4]))
    #2加载black_list.txt，形成RDD2，并collect形成list
    rdd2 = sc.textFile('file:///export/pyworkspace/pyspark_dev/综合案例/data/black_list.txt')
    ...
    ret_rdd1.coalesce(1).saveAsTextFile('hdfs://node1:8020/pydata/csv')

    #9加载user.txt形成DataFrame1
    df1=spark.read\
        .schema('id int,name string,age int,sex string,city string')\
        .csv('file:///export/pyworkspace/pyspark_dev/综合案例/data/user.txt')
    df1.printSchema()
    df1.show()
    #10加载black_list.txt形成DataFrame2
    df2=spark.read\
        .schema('id int,name string')\
        .csv('file:///export/pyworkspace/pyspark_dev/综合案例/data/black_list.txt')
    df2.printSchema()
    df2.show()
    ...
    df4.coalesce(1).write.mode('overwrite').option('header',True).csv('hdfs://node1:8020/pydata/csv2')
    df4.coalesce(1).write\
        .mode('overwrite')\
        .format('jdbc')\
        .option('url','jdbc:mysql://node1:3306/bigdata?serverTimezone=UTC&characterEncoding=utf8&useUnicode=true')\
        .option('dbtable','bank_report')\
        .option('user','root')\
        .option('password','123456')\
        .save()
    #16还要保存到hive中。
    df4.coalesce(1).write.saveAsTable('bigdata.bank_report')
    #还可以用SQL来插入hive
    df4.createOrReplaceTempView('bank_report_temp')
    spark.sql('drop table if exists bigdata.bank_report2')
    # sql中的hint语法  /*+ repartition(1) */
    spark.sql('create table bigdata.bank_report2 as select /*+ repartition(1) */ * from bank_report_temp')
    #17-程序最后放在yarn上运行。